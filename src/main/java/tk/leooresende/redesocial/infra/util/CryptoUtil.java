...
public class CryptoUtil {
	...
	public static String descriptografarAESComAKey(String payload) {
		try {
			...
			final byte[][] keyAndIV = CryptoUtil.GenerateKeyAndIVToDescript(32, 16, 1, saltData, SECRET_KEY_OUT.getBytes(StandardCharsets.UTF_8),
					md5);
	...
	public static String criptografar(String payload) {
        try {
            String password = CryptoUtil.SECRET_KEY_IN;
            SecureRandom sr = new SecureRandom();
            byte[] salt = new byte[8];
            sr.nextBytes(salt);
            final byte[][] keyAndIV = CryptoUtil.generateKeyAndIVToEncrypt(32, 16, 1, salt, password.getBytes(StandardCharsets.UTF_8),
	...
	public static String pegarEDescriptografarRequisicao(HttpServletRequest request) {
		CryptoRequestForm corpoDaRequisicao = CryptoUtil.pegarCorpoDaRequisicao(request);
		return CryptoUtil.descriptografarAESComAKey(corpoDaRequisicao.getPayload());
	}
	
	private static CryptoRequestForm pegarCorpoDaRequisicao(HttpServletRequest request) {
			String reqJsonCrypt = CryptoUtil.pegarCorpoDaRequisicaoEmString(request);
			Gson gson = new Gson();
	...
	public static String pegarParametroDescriptografado(String nomeDoParametro, HttpServletRequest request) {
		...
		return CryptoUtil.descriptografarAESComAKey(parametroValor);
	}
