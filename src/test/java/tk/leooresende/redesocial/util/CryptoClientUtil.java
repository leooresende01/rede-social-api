package tk.leooresende.redesocial.util;
...
public class CryptoClientUtil {
	...
	public static String descriptografarAESComAKey(String payload) {
		try {
			...
			final byte[][] keyAndIV = CryptoClientUtil.GenerateKeyAndIVToDescript(32, 16, 1, saltData,
					SECRET_KEY_IN.getBytes(StandardCharsets.UTF_8), md5);
	...
	private static byte[][] GenerateKeyAndIVToDescript(int keyLength, int ivLength, int iterations, byte[] salt,
	...
	public static String criptografar(String payload) {
		try {
			String password = CryptoClientUtil.SECRET_KEY_OUT;
			SecureRandom sr = new SecureRandom();
			byte[] salt = new byte[8];
			sr.nextBytes(salt);
			final byte[][] keyAndIV = CryptoClientUtil.generateKeyAndIVToEncrypt(32, 16, 1, salt,
					password.getBytes(StandardCharsets.UTF_8), MessageDigest.getInstance("MD5"));
	...
	public static byte[][] generateKeyAndIVToEncrypt(int keyLength, int ivLength, int iterations, byte[] salt,
			byte[] password, MessageDigest md) {
